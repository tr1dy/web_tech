...
export const fetchBooksByName = async (name) => {
  try {
    const response = await fetch(`https://openlibrary.org/search.json?q=${encodeURIComponent(name)}&limit=40&fields=*,subject`);
...
export const fetchBooksByCategory = async (category) => {
  try {
    const response = await fetch(`https://openlibrary.org/search.json?subject=${encodeURIComponent(category)}&limit=40&fields=*,subject`);
    const data = await response.json();
    return (data.docs || []).map(mapOpenLibraryToBook);
  } catch (error) {
